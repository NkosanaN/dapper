...
namespace DapperIdentityDemo.Identity
{
   ...
   
    {
        ...
        public virtual Task CreateAsync(User user)
        {
            ...
            {
                connection.Execute("insert into Users(UserId, UserName, PasswordHash, SecurityStamp , AccessLevel) values(@userId, @userName, @passwordHash, @securityStamp , @accessLevel)", user);
            }
            ...
            return Task.Factory.StartNew(() => AddToRoleAsync(user, "Individual"));   
        }
        ...
        public Task AddToRoleAsync(User user, string roleName)
        {
            ...
            return Task.Factory.StartNew(() =>
            {
                //NB:Nkosana //user.UserId = Guid.NewGuid();
                using (SqlConnection connection = new SqlConnection(connectionString))
                    connection.Execute("INSERT INTO UserRoles (UserId , RoleId) SELECT @userId, RoleId FROM Roles WHERE RoleName = @roleName", new { user.UserId, roleName });
            });
        }
        ...
        public virtual Task<IList<string>> GetRolesAsync(User user)
        {
            ...
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                    return (IList<string>)connection.Query<string>
                    ("SELECT r.RoleName FROM UserRoles AS ur JOIN Roles AS r ON ur.RoleId = r.RoleId WHERE ur.UserId = @userId", new { user.UserId }).ToList();
            });
        }
